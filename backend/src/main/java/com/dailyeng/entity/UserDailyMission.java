...
@Entity @Table(name = "\"UserDailyMission\"", uniqueConstraints = @UniqueConstraint(columnNames = {"userId", "missionId", "date"}))
...
public class UserDailyMission extends BaseEntity {
    ...
    @Column(nullable = false) private String missionId;
    @ManyToOne(fetch = FetchType.LAZY) @JoinColumn(name = "missionId", insertable = false, updatable = false) private DailyMission mission;
